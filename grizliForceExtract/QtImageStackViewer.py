...
class QtImageStackViewer(QWidget):
    ...
    def updateFrame(self):
        ...
        if type(self._image) is np.ndarray:
            ...
            print (indexes)
            print (rows)
            print (self.currentIndexes())
            print ([[i] for i in self.currentIndexes()])
            print ([rows].extend([[i] for i in self.currentIndexes()]))
            self._currentFrame = self._image[np.ix_(*indexes)][:,:,0]
            print (np.array(self._currentFrame))
