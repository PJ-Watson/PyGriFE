...
{
  objstruct	*obj = &objlist->obj[no];
  pliststruct	*pixel = objlist->plist, *pixt;  /* pixel is the object plist, pixt is the pointer structure*/
...
/* These functions are only called if an existing segmentation map is used. */
/* Well written or efficient code? No, but there are more productive things */
/* to do than rewrite this. */

void singleobjpreanalyse(int no, idinfostruct *objinfo, objliststruct *objlist)
{
  objstruct	*obj = &objlist->obj[no];
  pliststruct	*pixel = objlist->plist, *pixt;
  PIXTYPE	peak, cpeak, val, cval;
  ...
  printf("Reached pre-analysis, %d", no);
  fflush(stdout);
  ...
  for (it=0; it<objinfo->pixnb; it++) {
    ...
    val = PLISTPIX(pixt, value);
    cval = PLISTPIX(pixt, cdvalue);
    // printf("iteration %d / %d, (%d, %d)", it, objinfo->pixnb, x, y);
    // fflush(stdout);
    if (peak < val) {
      peak = val;
      xpeak = x;
    ...
    if (ymax < y)
	    ymax = y;
    thresh = PLISTPIX(pixt, thresh);
    // printf("T=%f", thresh);
    fdnpix++;
  }

  printf("Finished pre-analysis loop, %d", no);
  fflush(stdout);
...
void singleobjanalyse(int no, idinfostruct *objinfo, objliststruct *objlist, int robust, double gain)
{
  objstruct	*obj = &objlist->obj[no];
  pliststruct	*pixel = objlist->plist, *pixt;
  PIXTYPE	peak, val, cval;
  double	thresh,thresh2, t1t2,darea,
		...
		xm,ym, xm2,ym2,xym,
                temp,temp2, theta,pmx2,pmy2,
                errx2, erry2, errxy, cvar, cvarsum;
  int		x, y, xmin, ymin, area2, dnpix, it;

  printf("Reached this stage, %d", no);
  fflush(stdout);
  singleobjpreanalyse(no, objinfo, objlist);
  ...
  int k;
  double newthresh, thresh0;
  thresh0 = obj->thresh;
  // thresh =  0.0008460828685201705;

  printf("Current threshold: %f", thresh0);

  // for (k=1; k<32; k++) {
  //   /*------ Calculate threshold */
  //   thresh = obj->fdpeak;
  //   newthresh = thresh > 0.0 ? thresh0*pow(thresh/thresh0,(double)k/32) : thresh0;
  // }

  // printf("New threshold; %f", newthresh);
