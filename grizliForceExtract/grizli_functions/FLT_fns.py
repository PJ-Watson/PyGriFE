...
def mod_transform_JWST_WFSS(flt, verbose=True):
        ...
        if flt.catalog is not None:
            ...
            flt.catalog = flt.blot_catalog(flt.catalog,
                          sextractor=('X_WORLD' in flt.catalog.colnames))

def mod_compute_model_orders(self,
                             id=0,
                             x=None,
                             y=None,
                             size=10,
        ...
        if hasattr(self, "orig_seg"):
            for b in beams:
                ...
                slx_thumb = slice(beam.origin[1],
                            beam.origin[1]+beam.sh[1])
